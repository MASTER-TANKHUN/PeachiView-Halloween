...
import { loadPeachi, loadHeadphones } from './peachi3d.js';
...
export function buildHeadphonesItemProcedural() {
  ...
  const phones = new THREE.Group(); phones.name = 'phones'; phones.scale.setScalar(1.35); phones.rotation.x = -0.25; inner.add(phones);
  const bin = new PartBin();
...
// =====================================================================================
// Rigged 3D Peachi (assets/models/peachi.glb + headphones.glb, runtime in peachi3d.js, generated by tools/blender/).
// buildPeachi() / buildHeadphonesItem() keep the contracts above: they return the procedural model at once and
// swap in the GLB as soon as it has loaded (they stay procedural if loading fails, or with use3d: false).
// Every call gets its own instance; each GLB is downloaded once.
const EMOTION_OF = { happy: 'happy', cry: 'cry', angry: 'angry', scream: 'scream' };
const CLIP_OF = { idle: 'Idle', float: 'Float', reach: 'Reach', stunned: 'Stunned', jumpscare: 'Jumpscare' };
...
export function buildPeachi({ ghost = true, use3d = true } = {}) {
  ...
  // one stable anchor for callers that keep a reference: it rides on whichever model is shown
  const faceAnchor = new THREE.Object3D();
  proc.faceAnchor.add(faceAnchor);
  const st = { expr: 'happy', pose: 'float', glow: 0, dark: 0, desat: 0, color: null, phones: true, look: null };
  let p3 = null, playing = null, disposed = false;
  const play = (fade) => {
    const idle = st.pose === 'idle';
    const clip = idle && st.expr === 'cry' ? 'Cry' : idle && st.expr === 'angry' ? 'Angry' : CLIP_OF[st.pose];
    if (clip !== playing) { playing = clip; p3.play(clip, fade); }
  };
  ...
  loadPeachi().then((p) => {
    ...
    proc.dispose();
    group.add(p.object);
    p.faceAnchor.add(faceAnchor);
    p.setLookAt(false);
    p.setGhost(ghost ? 1 : 0);
    p.setEmotion(st.expr);
    p.setRim(0.35 + 1.2 * st.glow);
    if (st.color !== null) p.setGlowColor(st.color);
    p.setDark(st.dark);
    p.setDesat(st.desat);
    p.setHeadphones(st.phones);
    p.lookAt(st.look);
    play(0);
  }).catch((e) => console.warn('[peachi] 3D model failed to load, keeping the procedural one:', e));

  return {
    group,
    faceAnchor,
    get faceHeight() { return p3 ? p3.faceHeight : FACE_HEIGHT; },
    get expression() { return st.expr; },
    get pose() { return st.pose; },
    get dark() { return st.dark; },
    get headphones() { return st.phones; },
    get is3D() { return !!p3; },
    setExpression(name) {
      st.expr = EMOTION_OF[name] ? name : 'happy';
      if (p3) { p3.setEmotion(st.expr); play(); } else proc.setExpression(name);
    },
    setPose(name) {
      if (!CLIP_OF[name]) return;
      st.pose = name;
      if (p3) play(); else proc.setPose(name);
    },
    setGlow(v) {
      st.glow = THREE.MathUtils.clamp(v, 0, 1);
      if (p3) p3.setRim(0.35 + 1.2 * st.glow); else proc.setGlow(v);
    },
    setDark(v) { st.dark = THREE.MathUtils.clamp(v, 0, 1); (p3 || proc).setDark(st.dark); },
    setDesat(v) { st.desat = THREE.MathUtils.clamp(v, 0, 1); (p3 || proc).setDesat(st.desat); },
    setGlowColor(hex) { st.color = hex; (p3 || proc).setGlowColor(hex); },
    setHeadphones(on) { st.phones = !!on; (p3 || proc).setHeadphones(st.phones); },
    lookAt(v) { st.look = v ? (st.look || V()).copy(v) : null; (p3 || proc).lookAt(st.look); },
    update(dt, t) { if (p3) p3.update(dt, null); else proc.update(dt, t); },
    dispose() {
      disposed = true;
      faceAnchor.removeFromParent();
      if (p3) p3.dispose(); else proc.dispose();
    },
  };
}

export function buildHeadphonesItem({ use3d = true } = {}) {
  const group = buildHeadphonesItemProcedural();
  if (!use3d) return group;
  const phones = group.getObjectByName('phones');
  let item = null, disposed = false;
  loadHeadphones().then((h) => {
    if (disposed) { h.dispose(); return; }
    item = h;
    // same spot and width as the procedural headphones it replaces (their bounding box in `phones` space)
    group.updateMatrixWorld(true);
    const inv = phones.matrixWorld.clone().invert(), box = new THREE.Box3(), part = new THREE.Box3(), m = new THREE.Matrix4();
    phones.traverse((o) => {
      if (!o.isMesh) return;
      if (!o.geometry.boundingBox) o.geometry.computeBoundingBox();
      box.union(part.copy(o.geometry.boundingBox).applyMatrix4(m.multiplyMatrices(inv, o.matrixWorld)));
    });
    for (const c of phones.children) c.visible = false;
    const hb = new THREE.Box3().setFromObject(h.object);
    const s = (box.max.x - box.min.x) / (hb.max.x - hb.min.x);
    h.object.scale.setScalar(s);
    h.object.position.copy(box.getCenter(V())).sub(hb.getCenter(V()).multiplyScalar(s));
    phones.add(h.object);
  }).catch((e) => console.warn('[peachi] headphones model failed to load, keeping the procedural one:', e));
  const disposeProc = group.userData.dispose;
  group.userData.dispose = () => { disposed = true; if (item) item.dispose(); disposeProc(); };
  return group;
}
